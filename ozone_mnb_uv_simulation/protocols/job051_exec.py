#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""JOB-2026-0906-051: standard-BFGS limited continuation from the 050 last
accepted point (opt_20).
...
Reuses the validated evaluation & persistence components unchanged
(job047_exec Ledger / eval_point / gates / RealBackend); only the source,
the run directory and the record identifiers are adapted, plus the
charter-required separate bookkeeping:
  optimizer requests, completed evaluations, accepted iterates and
  budget-BLOCKED requests are recorded SEPARATELY; a blocked request
  (refused by the persisted pre-call guard, no energy/gradient) is never
  counted as a line-search rejection.

Method/units identical to 050: D2 attached once, grid level 8, original
SCF tolerances, grid_response=True; 21 Cartesian variables in Bohr;
scipy method='BFGS', jac=True, gtol=1e-6, norm=inf, xrtol=0, c1=1e-4,
c2=0.9, hess_inv0 = 21x21 identity, maxiter=200 (algorithm parameter only;
the real limit is the persisted opt cap).  Geometry continuation with
optimizer state RESET (050 saved no inverse-Hessian history).
"""
...
import job047_exec as ex          # validated chain
from scipy.optimize import minimize as _scipy_minimize

OUT = ROOT + '/run_artifacts/02_nh3o3_reference/c1_bfgs_cont051'
LEDGER = OUT + '/budget_bfgs_cont051.json'
RESULTS = OUT + '/bfgs_cont051_results.json'
...
class LoggingBackend:
    """Per-eval progress + exact request coordinates persisted in records."""
    ...
    def full_eval(self, R_bohr, out_dir, tag):
        ...
        print('[051] eval %s: SCF+gradient start' % tag, flush=True)
        t0 = time.time()
        rec = self.inner.full_eval(R, out_dir, tag)
        rec['x_bohr'] = R.tolist()
        rec['x_bohr_sha'] = sha_arr(R)
        rec['seconds'] = round(time.time() - t0, 1)
        print('[051] eval %s: E=%.9f grad_max=%.3e conv=%s (%.0fs)'
...
def _setup_note(maxiter):
    return ('scipy method=BFGS, jac=True, gtol=1e-6, norm=inf, xrtol=0, '
            ...
            'continuation with optimizer state RESET (050 saved no '
            'inverse-Hessian history); maxiter is NOT the evaluation '
            'budget (persisted opt cap guards every call)' % maxiter)
...
def main():
    ...
    if sha != man['source']['source_sha256']:
        raise RuntimeError('HARD STOP: 050 opt_20 source file hash changed')
    if not man['source']['all_checks_pass']:
        raise RuntimeError('HARD STOP: source prechecks failed')
    src = json.load(open(src_path))
    print('[051] source OK: %s (sha256 %.16s...)'
          % (os.path.basename(src_path), sha), flush=True)

    results = dict(
        job='JOB-2026-0906-051 standard-BFGS limited continuation from '
            'the 050 last accepted point (opt_20)',
        doing='continue from the 050 last accepted point opt_20 with ONE '
              'limited standard-BFGS run (same method/units as 050, fresh '
              'optimizer state) to test whether the unprojected '
              'max|g|<=1e-5 gate can be reached; independent recheck '
              'required for any candidate registration',
        source=dict(path=src_path, sha256=sha,
                    e_total=float(src['e_total']),
                    grad_max=float(src['grad_max']),
                    identity=dict(last_accepted_050=True,
                                  last_evaluated_050=True,
                                  lowest_energy_050=True),
    ...
    try:
        ...
        g = rec_s['gate']
        print('[051] start gate %s: dE=%.2e dgrad=%.2e dC=%.2e'
        ...
        print('[051] optimization outcome: %s (new evals %d, '
              'reuse_start %s, blocked requests %d)'
              % (opt['outcome'], opt['n_new_evals'], opt['reuse_start'],
                 len(opt['blocked_requests'])), flush=True)
        if opt.get('warnings'):
            print('[051] optimizer warnings: %s' % opt['warnings'],
                  flush=True)
        if opt['meeting_points']:
            ...
            try:
                ...
                if rec_r['gate']['gates_pass']:
                    ...
                    print('[051] recheck PASS -> registered: stationary '
                ...
                status = 'recheck_failed'
                print('[051] recheck FAILED -> not registered', flush=True)
        else:
            status = 'not_converged'
            print('[051] no saved point met max|g|<=1e-5 -> not converged',
                  flush=True)
    except Exception as exc:                                    # noqa: BLE001
        results['abort'] = dict(error=str(exc),
                                traceback=traceback.format_exc()[-1500:])
        status = 'aborted'
        print('[051] ABORT: %s' % exc, flush=True)
    ...
    results['final'] = dict(
        ...
        note='single limited continuation; no step-count forecast; no '
             'auto-continuation; stability & frequency acceptance NOT '
             'performed')
